...
const statusStyle: Record<string, { label: string; bg: string; color: string; border: string }> = {
  ACTIVE: { label: "Activa",     bg: "rgba(43,212,154,0.10)",  color: "#2bd49a", border: "rgba(43,212,154,0.20)" },
  PAUSED: { label: "Pausada",    bg: "rgba(251,191,36,0.10)",  color: "#fbbf24", border: "rgba(251,191,36,0.25)" },
  ENDED:  { label: "Finalizada", bg: "oklch(0.30 0.02 250)",   color: "oklch(0.72 0.01 250)", border: "oklch(0.30 0.02 250)" },
}
    ...
    <div className="space-y-6 max-w-6xl">
      ...
      {campaigns.length === 0 ? (
        <div className="rounded-2xl p-16 text-center" style={{ background: "oklch(0.19 0.015 250)", border: "1px solid oklch(0.30 0.02 250)" }}>
          ...
          <p style={{ color: "oklch(0.62 0.01 250)" }}>Sin campañas</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="min-w-0">
                  <p className="font-medium text-[13px] truncate" style={{ color: "#ffffff" }}>{c.title}</p>
                  {c.endDate && (
                ...
                <div>
                  <p className="text-[13px] truncate" style={{ color: "oklch(0.72 0.01 250)" }}>{c.business.name}</p>
                  <p className="text-[11px] mt-0.5" style={{ color: "oklch(0.62 0.01 250)" }}>{c.business.type}</p>
                </div>
                <span
                ...
                <p className="text-[12px] font-semibold" style={{ color: "#2bd49a" }}>
                  {incentiveText}
                </p>
