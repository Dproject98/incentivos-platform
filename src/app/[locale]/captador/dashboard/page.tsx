...
export default async function CaptadorDashboard() {
  ...
  const stats = [
    ...
    { label: t("my_reservations"),  value: reservations.length,                          icon: CalendarCheck, color: "#E8735A" },
    { label: t("confirmed"),        value: confirmedCount,                               icon: TrendingUp,   color: "#E8735A" },
    { label: t("balance"),          value: `${wallet?.balance.toFixed(2) ?? "0.00"} €`, icon: Wallet,       color: "#E8735A" },
  ]

  const statusConfig: Record<string, { label: string; bg: string; color: string; border: string }> = {
    PENDING:   { label: tRes("status_pending"),   bg: "rgba(217,179,108,0.10)",  color: "#D9B36C", border: "rgba(217,179,108,0.25)" },
    CONFIRMED: { label: tRes("status_confirmed"), bg: "rgba(232,115,90,0.10)",  color: "#E8735A", border: "rgba(232,115,90,0.20)" },
    CANCELLED: { label: tRes("status_cancelled"), bg: "rgba(220,38,38,0.12)",   color: "#dc2626", border: "rgba(220,38,38,0.20)" },
    NO_SHOW:   { label: tRes("status_no_show"),   bg: "#34373C",   color: "#AEB2B8", border: "#34373C" },
  }
    ...
    <div className="space-y-8 max-w-5xl">
      ...
      <div>
        ...
        <p className="text-[14px] mt-1" style={{ color: "#93979E" }}>
          Bienvenido de nuevo, <span style={{ color: "#E8735A", fontWeight: 500 }}>{session.user.name}</span>
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        >
          <div>
            <p className="font-semibold text-[15px]" style={{ color: "#F2F1EF" }}>Ver campañas</p>
            <p className="text-[13px] mt-1" style={{ color: "#93979E" }}>{activeCampaigns} campañas activas disponibles</p>
          </div>
        ...
        >
          <div>
            <p className="font-semibold text-[15px]" style={{ color: "#F2F1EF" }}>Mi wallet</p>
            <p className="text-[13px] mt-1" style={{ color: "#93979E" }}>
              Saldo: <span style={{ color: "#E8735A", fontWeight: 500 }}>{wallet?.balance.toFixed(2) ?? "0.00"} €</span>
            </p>
          </div>
          <ArrowRight className="h-5 w-5 shrink-0 transition-transform group-hover:translate-x-1" style={{ color: "#E8735A" }} />
        </Link>
      ...
      {reservations.length > 0 && (
      <div>
        <h2 className="font-semibold mb-4 text-[16px]" style={{ color: "#F2F1EF" }}>
          {t("recent")}
        </h2>
